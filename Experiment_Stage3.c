...
struct oc_device
{
    ...
    void(*cmd_rsp_cb)(uint8_t *topic_data, size_t topic_size,uint8_t *recv_data, size_t recv_size, uint8_t **resp_data, size_t *resp_size);
 ...
 void mqtt_callback(MessageData *msg_data)
{
    ...
    {
        oc_mqtt.cmd_rsp_cb((uint8_t *) msg_data->topicName->lenstring.data, msg_data->topicName->lenstring.len,(uint8_t *) msg_data->message->payload, msg_data->message->payloadlen, &response_buf,
                &res_len);
...
static int oc_mqtt_entry(void)
{
	...
	MQTTClientInit(&mq_client, &n, 3000, oc_mqtt_buf, buf_size, oc_mqtt_readbuf, buf_size);
 ...
 *         -1 : function is null
 */
void oc_set_cmd_rsp_cb(void (*cmd_rsp_cb)(uint8_t *topic_data, uint32_t topic_size,uint8_t *recv_data, uint32_t recv_size, uint8_t **resp_data, uint32_t *resp_size))
{
 ...
 int oc_mqtt_publish(char  *topic,uint8_t *msg,int msg_len,int qos)
{
    ...
    {
        if(MQTTIsConnected(&mq_client) != true){
            printf("mqtt disconnected, please check network or restart device!\n");
        }
        return -1;
    }
